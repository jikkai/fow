...
import { TILE_WIDTH_OFFSET, BITMAP_WIDTH_OFFSET, BITMAP_WIDTH } from '@/entries/constants'
import { makeKeyXY } from '@/utils/helpers'

const DEBUG = false
const FOW_TILE_ZOOM = 9
const FOW_BLOCK_ZOOM = FOW_TILE_ZOOM + TILE_WIDTH_OFFSET
...
class Internal {
  private static renderTileOnCanvas(
    ...
    const CANVAS_FOW_BLOCK_SIZE_OFFSET =
      tileSizeOffset - TILE_WIDTH_OFFSET
  ...
  static renderBlockOnCanvas(
    ...
    if (blockSizeOffset <= 0) {
      ...
      const CANVAS_FOW_PIXEL_SIZE_OFFSET =
        blockSizeOffset - BITMAP_WIDTH_OFFSET
    ...
    if (tileIndex.z <= FOW_TILE_ZOOM) {
      ...
      for (let fowTileX = fowTileXMin; fowTileX < fowTileXMax; fowTileX++) {
        for (let fowTileY = fowTileYMin; fowTileY < fowTileYMax; fowTileY++) {
          const fowTile =
            fogMap.tiles[makeKeyXY(fowTileX, fowTileY)]
      ...
      if (tileIndex.z > FOW_BLOCK_ZOOM) {
        ...
        const subBlockMask =
          (1 << (TILE_OVER_OFFSET - TILE_WIDTH_OFFSET)) - 1

        const CANVAS_NUM_FOW_PIXEL_OFFSET =
          CANVAS_NUM_FOW_BLOCK_OFFSET + BITMAP_WIDTH_OFFSET
        ...
        const block =
          fogMap.tiles[makeKeyXY(fowTileX, fowTileY)]?.blocks[
            makeKeyXY(fowBlockX, fowBlockY)
          ]
